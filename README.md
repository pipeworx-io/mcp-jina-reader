...
| `search` | Web search returning the top results as markdown. Higher-volume usage may require your own Jina API key (pass via _apiKey). |
...
```json
{
  "mcpServers": {
    "jina_reader": {
      "url": "https://gateway.pipeworx.io/jina_reader/mcp"
    }
  }
}
```
...
`tools/list` at `https://gateway.pipeworx.io/jina_reader/mcp` returns the tools in the table
